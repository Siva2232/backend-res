...
const getLeaves = async (req, res) => {
  try {
    ...
    const total = await HRLeave.countDocuments(query);
    const leaves = await HRLeave.find(query)
...
const applyLeave = async (req, res) => {
  try {
    ...
    if (req.hrStaff) {
      ...
      staffDoc = await HRStaff.findOne({ email: req.user.email });
...
const getMyLeaves = async (req, res) => {
  try {
    ...
    if (req.hrStaff) {
      ...
      const HRStaff = require('../models/HRStaff');
      const staffDoc = await HRStaff.findOne({ email: req.user.email });
      if (!staffDoc) return res.status(404).json({ message: 'No HR staff profile found for this account' });

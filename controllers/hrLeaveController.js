...
const HRLeave = async (req) => getModel('HRLeave', HRLeaveModel.schema, req.restaurantId);
const HRStaffBaseModel = require('../models/HRStaff');
const HRStaff = async (req) => getModel('HRStaff', HRStaffBaseModel.schema, req.restaurantId);
...
const getLeaves = async (req, res) => {
  try {
    ...
    const total = await (await HRLeave(req)).countDocuments(query);
    const leaves = await (await HRLeave(req)).find(query)
...
const applyLeave = async (req, res) => {
  try {
    ...
    if (req.hrStaff) {
      ...
      staffDoc = await (await HRStaff(req)).findOne({ email: req.user.email });
    ...
    const leave = await (await HRLeave(req)).create({ staff: staffId, type, startDate, endDate, reason });
    const populated = await (await HRLeave(req)).findById(leave._id).populate('staff', 'name email');
...
const updateLeave = async (req, res) => {
  try {
    ...
    const leave = await (await HRLeave(req)).findById(req.params.id);
    ...
    await leave.save();
    const populated = await (await HRLeave(req)).findById(leave._id)
      .populate('staff', 'name email department')
      .populate('reviewedBy', 'name');
    emitUpdate(req, 'leaveUpdate', populated);
    res.json(populated);
...
const getMyLeaves = async (req, res) => {
  try {
    ...
    if (req.hrStaff) {
      ...
      const staffDoc = await (await HRStaff(req)).findOne({ email: req.user.email });
      if (!staffDoc) return res.status(404).json({ message: 'No HR staff profile found for this account' });

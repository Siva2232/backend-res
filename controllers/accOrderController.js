...
const createOrder = async (req, res) => {
  try {
    ...
    const saved = await createLedgerEntries(entries, 'AccOrder', order._id, partyId, null, req.restaurantId);
...
const updateOrder = async (req, res) => {
  try {
    ...
    const entries = await buildSalesEntries({ totalAmount, paidAmount, balance, paymentMode, date: date ? new Date(date) : new Date() , restaurantId: req.restaurantId, restaurantId: req.restaurantId});
    const saved = await createLedgerEntries(entries, 'AccOrder', order._id, order.party, null, req.restaurantId);
    order.ledgerEntries = saved.map(e => e._id);
    await order.save();
